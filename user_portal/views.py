...
def user_makereservation(request):
    ...
    if request.method == 'POST':
        ...
        # Set date_reserved automatically
        date_reserved = timezone.now().date()
        ...
        required_fields = [organization, representative, contact_number, start_time, end_time, event_type]
        if not all(required_fields) or not dates or not facilities:
            messages.error(request, 'Please fill in all required fields.')
            return redirect('user_portal:user_makereservation')
        temp_reservation = Reservation()
        for date in dates:
            if temp_reservation.is_date_blocked(date, facilities[0]):
                messages.error(request, f'The selected date ({date}) is already reserved for {facilities[0]}. Please choose another date.')
                return redirect('user_portal:user_makereservation')
        try:
            ...
            return redirect('user_portal:user_makereservation')
        # Only require that start and end time are not the same
        if start_time == end_time:
            messages.error(request, 'Start and end time cannot be the same.')
            return redirect('user_portal:user_makereservation')
        facility_keys = [
...
def edit_reservation(request, id):
    ...
    if request.method == 'POST':
        # Update actual use dates (multi-date)
        dates_raw = request.POST.get('dates')
        if dates_raw:
            reservation.dates = [d.strip() for d in dates_raw.split(',') if d.strip()]
        else:
            reservation.dates = []
    ...
    # Pass dates as comma-separated string for the multi-date picker
    dates_str = ','.join(reservation.dates) if reservation.dates else ''
...
def get_user_calendar_events(request):
    ...
    for reservation in approved_reservations:
        # Use the first date in reservation.dates if available
        event_date = reservation.dates[0] if reservation.dates and len(reservation.dates) > 0 else None
        events.append({
            'id': reservation.id,
            'title': f"{reservation.facility_use} - {reservation.organization}",
            'start': f"{event_date}T{reservation.start_time}" if event_date else None,
            'end': f"{event_date}T{reservation.end_time}" if event_date else None,
            'extendedProps': {
...
def get_reservations(request):
    ...
    for r in reservations:
        ...
        # Use the first date in r.dates if available
        event_date = r.dates[0] if r.dates and len(r.dates) > 0 else None
        events.append({
            'title': r.facility_use,  # assuming facility_use is a string
            'start': f"{event_date}T{r.start_time}" if event_date else None,
            'end': f"{event_date}T{r.end_time}" if event_date else None,

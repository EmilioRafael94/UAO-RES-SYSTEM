...
class Reservation(models.Model):
    ...
    # New fields for letter upload, facilities, and multiple dates
    # letter = models.FileField(upload_to='reservation_letters/', null=True, blank=False)  # Required letter upload
    facilities = models.ManyToManyField('superuser_portal.Facility', related_name='reservations')  # Multiple facilities, correct app reference
    dates = JSONField(default=list, blank=True)  # Replace ArrayField with JSONField for SQLite compatibility
    ...
    def check_date_availability(self, date, facility):
        """Check if a date is available for reservation (SQLite compatible)"""
        # Get all reservations for the facility with relevant statuses
        existing_reservations = Reservation.objects.filter(
            facility_use=facility,
            status__in=['Approved', 'Admin Approved', 'Payment Approved', 'Security Pass Issued', 'Completed']
        )
        # Manually check if the date is in the dates list
        for res in existing_reservations:
            if res.dates and date in res.dates:
                return False
        return True

    def is_date_blocked(self, date, facility):
        """Check if date is blocked for a specific facility (SQLite compatible)"""
